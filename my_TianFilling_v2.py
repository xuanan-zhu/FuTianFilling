...
def extract_love_info(text):
    ...
    if raw_type:
        # 1. 先尝试匹配带有括号的格式 (如：爱心（2份）)
        match = re.match(r"(.*?)[\(（](.*?)[\)）]", raw_type)
        if match:
            result["类型"] = match.group(1).strip()
            result["份数"] = match.group(2).replace("份", "").strip()
        else:
            # 2. 如果没有括号，尝试直接提取 "数字+份" (如：2份爱心)
            m_amount = re.search(r"(\d+)\s*份", raw_type)
            if m_amount:
                result["份数"] = m_amount.group(1)
                # 把 "2份" 删掉，剩下的就是类型
                result["类型"] = raw_type.replace(m_amount.group(0), "").strip()
...
def parse_key_value_text(text, field_alias):
    ...
    for raw_line in text.replace("\r\n", "\n").split("\n"):
        ...
        line = line.replace("【🔔爱心流动表】", "").strip() # 兼容不同标题
        if not line: continue
...
class AutoFillerApp:
    def __init__(self, root):
        ...
        self.mode_var = tk.IntVar(value=1) # 默认选中类型1
    ...
    def run_append(self):
        ...
        mode = self.mode_var.get()
        if not path or not os.path.exists(path): return messagebox.showerror("错误", "请先选择或新建 Excel 文件！")
        if not text: return
        ...
        try:
            ...
            for cell in sheet[1]:
                if cell.value:
                    first_header = str(cell.value).strip()
                    break
        except Exception:
            pass

        # 执行超强鲁棒性切分引擎
        records = self.smart_split_records(text, mode, first_header)
        ...
        try:
            for record in records:
                info = append_to_excel_safe(path, record, mode)
                self.add_to_history(info, mode)
                
                name = "未知"
                if mode == 1: name = info.get("真实姓名") or "未知"
                elif mode == 2: name = info.get("被流动人") or "未知"
                else: 
                    vals = [v for v in info.values() if v]
                    name = vals[0] if vals else "记录"
                    
                names.append(name)
                success_count += 1
            
            messagebox.showinfo("成功", f"批量处理完成！\n共成功写入 {success_count} 条记录：\n{', '.join(names)}")
            self.text_input.delete("1.0", tk.END)
        except Exception as e:
            messagebox.showerror("错误", f"写入被中断！已成功添加 {success_count} 条。\n错误详情: {str(e)}")
    ...
    def smart_split_records(self, text, mode, first_header=None):
        """
        超高鲁棒性的多样本切分引擎（已修复过早切断的Bug）
        """
        primary_keys = set()
        if mode == 1:
            primary_keys = {"真实姓名", "姓名", "团队"} 
        elif mode == 2:
            primary_keys = {"被流动人", "被流动学员"}
        elif mode == 3 and first_header:
            clean_fh = first_header.split("（")[0].split("(")[0].strip().replace(" ", "").replace(" ", "")
            if clean_fh:
                primary_keys = {clean_fh}
        ...
        for line in lines:
            ...
            if "：" in stripped or ":" in stripped:
                split_char = "：" if "：" in stripped else ":"
                possible_key = stripped.split(split_char, 1)[0].split("（")[0].split("(")[0].strip().replace(" ", "")
                if len(possible_key) <= 15:
                    key = possible_key

            if key:
                # 策略2：遇到重复键
                # 策略3：遇到主键（并且前提是当前记录已经收集过其他有效键，避免把表头和主键硬生生拆开）
                if key in seen_keys or (key in primary_keys and len(seen_keys) > 0):
                    if any(l.strip() for l in current_lines):
                        records.append("\n".join(current_lines))
                    current_lines = [line]
                    seen_keys = {key}
